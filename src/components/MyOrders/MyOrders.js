    ...
    swal({
      ...
      if (willDelete) {
        const handleDelete = (id) => {
          const url = `https://roam-server.onrender.com/myorders/${id}`;
          fetch(url, {
            method: "DELETE",
          })
            .then((res) => res.json())
  ...
  return (
    <div className="mt-5">
      <div className="chose-head">
        ...
        <h2 className="chose2">
          <span className="span-bg">Manage</span> Your Destination Plans
        </h2>
      </div>
              ...
              {exactItems.map((exactItem) => (
                <tr key={exactItem?._id}>
                  ...
                  <td className="text-center">
                    {" "}
                    <button
                      onClick={() => handleConfirm(exactItem?._id)}
